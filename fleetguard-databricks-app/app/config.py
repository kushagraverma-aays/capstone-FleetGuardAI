...
class Settings(BaseSettings):
    ...
    # ON, so a plain connection is refused outright. Off by default because the
    # local docker-compose server and a developer's own MySQL do not need it,
    # and a managed instance turns it on with one environment variable.
    ...
    @computed_field
    @property
    def db_connect_args(self) -> dict[str, dict[str, bool]]:
        """Driver-level arguments shared by the app engine and by Alembic.

        PyMySQL has no plain "use TLS" switch: it starts a TLS handshake as soon
        as the ssl mapping is non-empty, whatever the mapping contains. The
        placeholder key is the documented idiom for asking for TLS against a
        server whose certificate is signed by a CA the host already trusts,
        which is the case for Azure's managed MySQL.
        """
